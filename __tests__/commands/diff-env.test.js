...
describe('diffEnvCommand — @ref replacement', () => {
  ...
  test('replaces run-function widget URL with @ref when states is an array', async () => {
    // Real Twilio Studio v2 API returns states as an ARRAY
    const sourceServerless = {
      fetchedAt: '2026-02-28T00:00:00.000Z',
      resources: [
        {
  ...
  test('replaces run-function widget URL when fn.path is missing (uses friendlyName)', async () => {
    const sourceServerless = {
      fetchedAt: '2026-02-28T00:00:00.000Z',
      resources: [
        {
          ...
          functions: [{ sid: 'ZH_SRC', friendlyName: '/handler' }],
        },
      ],
    };
    const sourceStudioFlows = {
      ...
      resources: [
        {
          ...
          functions: [{ sid: 'ZH_TGT', friendlyName: '/handler' }],
        },
      ],
    };
    const targetStudioFlows = {
    ...
    // Flows are logically identical — only env-specific SIDs/URLs differ.
    // Functions have no path field, only friendlyName — should still work.
    expect(mockWriteJson).not.toHaveBeenCalled();
  });
